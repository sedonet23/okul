...
(function(){
  ...
  function kartRenderEt(veri){
    ...
    html += '<div>';
    html += '<div style="font-size:24px;font-weight:700;">' + Math.round(veri.current.temperature_2m) + '°C</div>';
    html += '<div style="color:var(--ink-muted);font-size:13px;">' + anlikBilgi.t + '</div>';
    html += '</div>';
    html += '</div>';
    ...
    for(var i=0; i<veri.daily.time.length; i++){
      ...
      html += '<div style="text-align:center;padding:8px 2px;border-radius:10px;background:rgba(128,128,128,.08);">';
      html += '<div style="font-size:12px;color:var(--ink-muted);margin-bottom:4px;">' + gunEtiketi(veri.daily.time[i], i) + '</div>';
      html += '<div style="font-size:20px;">' + gunBilgi.e + '</div>';
      html += '<div style="font-size:12px;margin-top:4px;"><strong>' + Math.round(veri.daily.temperature_2m_max[i]) + '°</strong><span style="color:var(--ink-muted);"> / ' + Math.round(veri.daily.temperature_2m_min[i]) + '°</span></div>';
      html += '</div>';
    }
    html += '</div>';

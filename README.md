...
- **Sosyal Kulüpler — Öğrenci Ataması**: Her öğrenciye (Sınıflar > öğrenci formu) tek bir sosyal kulüp seçilebiliyor. Kulüp tanımına "bu kulübü hangi sınıflar seçebilir" kısıtı eklendi (sınıf checkbox listesi + Tüm İlkokul/Tüm Ortaokul hızlı seçim butonları); kısıt boş bırakılırsa tüm sınıflar seçebilir. Öğrencinin sınıf listesindeki satırında ve detayında kulüp rozeti gösterilir.
- **Kulüp danışmanları kendi kulübüne öğrenci ekleyebiliyor**: Danışman öğretmenler, genel öğrenci düzenleme yetkileri olmasa bile — Profilim > "Diğer Görevlerim (Kulüp, ŞÖK, Zümre, Maarif...)" ekranından kendi kulüplerine "➕ Öğrenci Ekle" ile öğrenci ekleyip çıkarabilir (sınıf sınıf açılıp kapanan bir işaretleme listesiyle). Bu yetki dar kapsamlıdır: sadece kulüp ataması yazılabilir, öğrencinin başka hiçbir bilgisine (isim, telefon, veli vb.) bu yoldan dokunulamaz.
- **Kulüp bazlı öğrenci listesi raporu**: Bir kulübe atanmış tüm öğrenciler (tüm sınıflar genelinde) okul başlıklı, danışman öğretmen imza alanlı bir sayfada listelenip yazdırılabilir/PDF alınabilir. Genel "Öğrenci Listesi" raporuna da kulüp filtresi ve kulüp sütunu eklendi. Arama sayfasına, servis/sınıf filtreleriyle aynı desende bir "Sosyal Kulüp(ler)" filtresi eklendi.
- **Sınıf İçi Oturma Planı düzeltmeleri ve iyileştirmeleri**: "Masaları Kilitle" artık sadece masa konumunu/boyutunu dondurur, koltuk atama/boşaltma işlemlerini engellemez; koltuğa dokunurken oluşan "hayalet tıklama" (aynı anda yanlış öğeye işlem yapma) sorunu giderildi; kaydedilip yeniden açılan planlarda masa boyutlarının kaybolması (eski kayıtlarda otomatik onarımla) düzeltildi; varsayılan pencere sayısı sabit 3'e çekildi; öğretmen adı yazısı büyütüldü; PDF çıktısına okul logosu ve adı eklendi.

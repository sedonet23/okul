...
async function nobetOtomatikDagitimUygula() {
  ...
  /* ── HER ÖĞRETMENİN BAŞLANGIÇ PARAMETRELERİNİ BELIRLE ──
     Her öğretmen için:
     - Son gerçek nöbetini bul (Firestore'daki en son atama)
     - Son nöbeti Bahçe ise → bu aydaki ilk nöbeti Bina (tersine geçiş)
     - Son nöbeti Bina  ise → bu aydaki ilk nöbeti Bahçe
     - Hiç nöbeti yoksa   → referans haftasındaki yer (ilk nöbet)
     
     "Nöbet sayısı" değil "son yer" takibi yapıyoruz:
     Her nöbet tutulduğunda → sonraki nöbet ters yer
     Tatil → nöbet tutulmadı → sonraki nöbet AYNI yer (devam)
  */
  ...
  // Her öğretmenin en son atanma tarihi ve yeri
  ...
  // Her öğretmen için bu aydaki ilk nöbet yeri:
  // Son nöbet Bahçe → ilk nöbet Bina, Son nöbet Bina → ilk nöbet Bahçe
  // Hiç nöbet yok → referans haftasındaki yer
  const ogrIlkYer = {}; // ogretmenId → yerId (bu ayın ilk nöbet yeri)
  for (let g = 1; g <= 5; g++) {
    const ref = referansHafta[g];
    for (const ogrId of [ref.bahce, ref.bina]) {
      if (ogrId in ogrIlkYer) continue; // zaten belirlendi
      const son = ogrSonAtama[ogrId];
      if (!son) {
        // Hiç nöbeti yok → referans haftasındaki yer
        ogrIlkYer[ogrId] = (ref.bahce === ogrId) ? bahceYerId : binaYerId;
      } else {
        // Son nöbet tersine çevir
